...
FallExtraParcel='
 # Latent variables
  ...
  P =~ MF+CS+TO
  Count ~ P+BSCS+GRIT+EXTRA
'
fit.FallExtraParcel=cfa(FallExtraParcel, data=fallactivity, sample.nobs = nrow(fallactivity))
# display summary output
summary(fit.FallExtraParcel, fit.measures=TRUE, standardized=TRUE)
#get standardized variances
standardizedSolution(fit.FallExtraParcel)

#get a visual of the path
semPaths(fit.FallExtraParcel, what="est", layout="tree", title=TRUE, style="LISREL")
...
FallConsciParcel='
 # Latent variables
  ...
  P =~ MF+CS+TO
  Count ~ P+BSCS+GRIT+CONSCI
'
fit.FallConsciParcel=cfa(FallConsciParcel, data=fallactivity, sample.nobs = nrow(fallactivity))
# display summary output
summary(fit.FallConsciParcel, fit.measures=TRUE, standardized=TRUE)
#get standardized variances
standardizedSolution(fit.FallConsciParcel)

#get a visual of the path
semPaths(fit.FallConsciParcel, what="est", layout="tree", title=TRUE, style="LISREL")
```
Run the winter models.
```{r winter models}
...
WinterExtraParcel='
 # Latent variables
  ...
  EXTRA =~ EXTRAP1+EXTRAP2+EXTRAP3
  P =~ MF+CS+TO
  Count ~ P+EXTRA+BSCS+GRIT
'
fit.WinterExtraParcel=cfa(WinterExtraParcel, data=winteractivity, sample.nobs = nrow(fallactivity))
# display summary output
summary(fit.WinterExtraParcel, fit.measures=TRUE, standardized=TRUE)
#get standardized variances
standardizedSolution(fit.WinterExtraParcel)

#get a visual of the path
semPaths(fit.WinterExtraParcel, what="est", layout="tree", title=TRUE, style="LISREL")

### Now for the consci model
#establish latent variables and indicators, build model
WinterConsciParcel='
 # Latent variables
  TO =~ TOP1+TOP2+TOP3
  ...
  CONSCI =~ CONSCIP1+CONSCIP2+CONSCIP3
  P =~ MF+CS+TO
  Count ~ P+CONSCI+BSCS+GRIT
'
fit.WinterConsciParcel=cfa(WinterConsciParcel, data=winteractivity, sample.nobs = nrow(fallactivity))
# display summary output
summary(fit.WinterConsciParcel, fit.measures=TRUE, standardized=TRUE)
#get standardized variances
standardizedSolution(fit.WinterConsciParcel)

#get a visual of the path
semPaths(fit.WinterConsciParcel, what="est", layout="tree", title=TRUE, style="LISREL")
```